...
export function RapportsContent({ data }: { data: ReportData | null }) {
  if (!data || !data.stats) {
    ...
    <Tabs defaultValue="finances">
      ...
      <TabsContent value="finances" className="mt-4 space-y-4">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: "Ventes du mois", value: formatCurrency(stats.ventesDuMois), color: "text-blue-700" },
            { label: "Dépenses du mois", value: formatCurrency(stats.depensesDuMois), color: "text-red-600" },
            { label: "Bénéfice brut", value: formatCurrency(stats.beneficeBrut), color: stats.beneficeBrut >= 0 ? "text-green-600" : "text-red-600" },
            { label: "Paiements reçus", value: formatCurrency(stats.paiementsRecus), color: "text-green-600" },
          ].map((kpi) => (
        ...
        <Card className="border-0 shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-base">Évolution financière (6 derniers mois)</CardTitle>
          </CardHeader>
          <CardContent>
